...
public class CsvFileWriter implements CsvWriter {
	...
	public static CsvFileWriter createCsvFileWriter(OutputStream outputStream, char separator) {
		return createCsvFileWriter(new OutputStreamWriter(outputStream), separator);
	}

	public static CsvFileWriter createCsvFileWriter(File csvFile, char separator) {
		try {
			return createCsvFileWriter(new FileWriter(csvFile), separator);
		} catch (IOException e) {
			throw new CsvException("Error creating CSV file writer", e);
		}
	}

	public static CsvFileWriter createCsvFileWriter(Writer writer, char separator) {
		return createCsvFileWriter(writer, separator, CSVWriter.DEFAULT_QUOTE_CHARACTER);
	}
